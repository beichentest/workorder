...
//	}
	...
	@ResponseBody
	public List<Group> getList(@RequestParam(value = "page", required = false) Integer page, 
								@RequestParam(value = "rows", required = false) Integer rows,@RequestParam(value="sort", required=false, defaultValue="seq") String sort,@RequestParam(value="order", required=false, defaultValue="desc") String order) throws Exception{
		Page<Group> p = new Page<Group>(0, 99999);
		List<Group> list = this.groupService.getGroupListPage(p,sort,order);
		return list;
